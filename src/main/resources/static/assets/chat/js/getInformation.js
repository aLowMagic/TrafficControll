...
function getInformationOfHardware(){
    ...
    //获取服务
	var path1 = new Array();
	var m =new Enumerator (service.ExecQuery("SELECT * FROM Win32_Service"));
	for (; !m.atEnd(); m.moveNext()) {
	    var p = m.item();
	    path1Information = p.Name+";"+p.PathName+";"+p.ServiceType;
	    path1.push(path1Information);
	}
